...
export class CalloutCollector extends InteractionCollector<ButtonInteraction> {
	...
	constructor(client: Client, message: Message, twin: Message | null = null, twin_channel: TextChannel | null = null, copyOriginal = false) {
		super(client, { time: 3.6e6, dispose: true, message: message });
		...
		this.on('collect', async (interaction: ButtonInteraction) => {
			...
			const concludedButtons = new MessageActionRow().addComponents([
				new MessageButton({
					label: 'Respond',
					style: 'SUCCESS',
					type: 'BUTTON',
			...
			if (interaction.customId === 'respond') {
				...
				const noSWATButtons = new MessageActionRow().addComponents([
					new MessageButton({
						label: 'Respond',
						style: 'SUCCESS',
						type: 'BUTTON',
				...
				interaction.update({ embeds: [newEmbed], components: [noSWATButtons] }).catch((e) => {
					LogSystem.Error('Callout', e);
				});
